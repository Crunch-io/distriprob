...
export class InvDiscreteQuantile {

  private static distribution_quantile_finder(simplifiedCDF: (x: number,
   ...
   */
  private static adjust_bounds(bounds: {a: number, b: number},
                               type: "equal_nearest_integer" |
                                                "equal_ceil" |
                                               "equal_floor" |
                                                      "real"):
  void {
  ...
  private static getTol(type: "equal_nearest_integer" |
                                         "equal_ceil" |
                                        "equal_floor" |
                                               "real"):
  Tol {
   ...
   */
  private static do_inverse_discrete_quantile(simplifiedCDF: (x: number,
                                                              lowerTail: boolean)=>number,
                                              p: number,
                                              comp: boolean,
                                              guess: number,
                                              ...
                                              min_bound: number):
  number {
    const f = InvDiscreteQuantile.distribution_quantile_finder(simplifiedCDF, p, comp);
    const tol = InvDiscreteQuantile.getTol(tolType);
    ...
    //
    const r = Toms748.toms748_solve(f, bounds.a, bounds.b, fa, fb, tol, count);
  ...
  private static round_to_floor(simplifiedCDF: (x: number, lowerTail: boolean) => number,
                                result: number,
                                p: number,
                                c: boolean,
                                max_bound: number,
                                min_bound: number):
  number {
    let cc = Math.ceil(result);
    let pp = cc <= max_bound ? c ? simplifiedCDF(cc, false) : simplifiedCDF(cc, true) : 1;
  ...
  private static round_to_ceil(simplifiedCDF: (x: number, lowerTail: boolean) => number,
                                result: number,
                                p: number,
                                c: boolean,
                                max_bound: number,
                                min_bound: number):
  number {
    let cc = Math.floor(result);
    let pp = cc >= min_bound ? c ? simplifiedCDF(cc, false) : simplifiedCDF(cc, true) : 0;
  ...
  public static inverse_discrete_quantile(simplifiedPDF: (x: number) => number,
                                          simplifiedCDF: (x: number,
                                                          lowerTail: boolean) => number,
                                          p: number,
                                          c: boolean,
                                          guess: number,
  ...
  number {
    ...
    if (round === "up") {
      const result = InvDiscreteQuantile.do_inverse_discrete_quantile(
        simplifiedCDF,
        p,
        c,
        Math.ceil(guess),
      ...
      return InvDiscreteQuantile.round_to_ceil(
        simplifiedCDF,
        result,
        p,
        c,
      ...
      return InvDiscreteQuantile.round_to_floor(
        simplifiedCDF,
        result,
        p,
        c,
      ...
      return InvDiscreteQuantile.do_inverse_discrete_quantile(
        simplifiedCDF,
        p,
        c,
        guess,
