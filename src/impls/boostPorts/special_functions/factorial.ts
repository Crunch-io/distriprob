...
export class Factorial {
  ...
  public static double_factorial(n: number): number {
    ...
    if(n & 1) {
      ...
      //
      const result = Gamma.tgamma(n / 2 + 1) / Constants.SQRTPI();
      ...
      if (i < UncheckedFactorial.max_factorial()) {
        const result = UncheckedFactorial.unchecked_factorial(i);
  ...
  public static rising_factorial(x: number, n: number): number {
    ...
    if(x < 0) {
      ...
      let result = ((n&1) ? -1 : 1) * Factorial.falling_factorial(-x, n);
      if(inv) {
        result = 1 / result;
      }
    ...
    if((x < 1) && (x + n < 0)) {
      const val = Gamma.tgamma_delta_ratio(1 - x, -n);
      return (n & 1) ? -val : val;
    }
    //
  ...
  public static falling_factorial(x: number, n: number): number {
    ...
    if(x < 0.5){
      ...
      if(n > UncheckedFactorial.max_factorial() - 2){
        ...
        const t1 = x * Factorial.falling_factorial(
          x - 1,
          UncheckedFactorial.max_factorial() - 2
        );
        const t2 = Factorial.falling_factorial(
          x - UncheckedFactorial.max_factorial() + 1,
          n - UncheckedFactorial.max_factorial() + 1
        );
    ...
    if(x <= n - 1) {
      ...
      //
      const xp1 = x + 1;
      let n2 = Math.trunc(Math.floor(xp1));

      if(n2 === xp1) {
        return 0;
      }

      let result = Gamma.tgamma_delta_ratio(xp1, -n2);
      x -= n2;
      result *= x;
      ++n2;
