...
const BesselJYDerivativesAsym =
  require("./detail/bessel_jy_derivatives_asym").BesselJYDerivativesAsym;
const BesselJYDerivativesSeries
  = require("./detail/bessel_jy_derivatives_series").BesselJYDerivativesSeries;
const BesselDerivativesLinear = require("./detail/bessel_derivatives_linear").BesselDerivativesLinear;
const Precision = require("../tools/precision").Precision;


export class BoostBesselPrime {

  public static cyl_bessel_j_prime(v: number, x: number): number {
    //
    ...
    if (x === 0) {
      if (v === 1) {
        ...
        throw new Error(`Domain error: Got x = ${x}, but function is indeterminate${""
        } for this order`);
      }
    }
    //
    ...
    if ((Math.abs(x) < 5) || (Math.abs(v) > x * x / 4)) {
      ...
      const r = BesselJYDerivativesSeries.bessel_j_derivative_small_z_series(v, x);
      return inversed ? -r : r;
    }
  ...
  public static sph_bessel_j_prime(v: number, x: number): number {
    //
    // Prevent complex result:
    //
    if (x < 0) {
    ...
    //
    if (x === 0) {
      throw new Error(`Domain error: Got x = ${x}, but function is indeterminate for ${""
      }this order`);
    }
  ...
  public static cyl_bessel_i_prime(v: number, x: number): number {
    //
    // Prevent complex result:
    //
    if (x < 0 && Math.floor(v) !== v) {
  ...
  public static cyl_bessel_k_prime(v: number, x: number): number {
    //
  ...
  public static cyl_neumann_prime(v: number, x: number): number {
    //
    // Prevent complex and indeterminate results:
    //
    if (x <= 0) {
  ...
  public static sph_neumann_prime(v: number, x: number): number {
    //
