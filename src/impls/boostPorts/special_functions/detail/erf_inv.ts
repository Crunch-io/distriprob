...
export class ErfInv {
   ...
   */
  private static erf_inv_imp(p: number, q: number): number {
    let result = 0;
    ...
    if(p <= 0.5) {
      ...
      const g = p * (p + 10);
      const r = Rational.evaluate_polynomial(P, p) /
      ...
      const xs = q - 0.25;
      const r = Rational.evaluate_polynomial(P, xs) /
      ...
      if(x < 3) {
        ...
        const xs = x - 6;
        const R = Rational.evaluate_polynomial(P, xs) / Rational.evaluate_polynomial(Q, xs);
        result = Y * x + R * x;
      } else if(x < 44) {
  ...
  public static erfc_inv(z: number): number {
    //
    // Begin by testing for domain errors, and other special cases:
    //
    if((z < 0) || (z > 2)) {
      throw new Error(`Domain error: Argument outside range [0,2] in inverse erfc${""
      } function (got p=${z}).`);
    }
    if(z === 0 || z === 2) {
  ...
  public static erf_inv(z: number): number {
    //
    // Begin by testing for domain errors, and other special cases:
    //
    if((z < -1) || (z > 1)) {
      throw new Error(`Domain error: Argument outside range [-1, 1] in inverse erf${""
      } function (got p=${z}).`);
    }
    if(z === 1 || z === -1) {
      throw new Error(`Overflow error`);
