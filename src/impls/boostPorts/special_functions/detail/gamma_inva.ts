...
export class GammaInvA {

  private static gamma_inva_t(z: number, p: number, invert: boolean):
  (a: number) => number {
    return (a: number) => {
  ...
  public static inverse_poisson_cornish_fisher(lambda: number,
                                                p: number,
                                                q: number):
  number {
    // mean:
  ...
  private static gamma_inva_imp(z: number, p: number, q: number): number {
    //
    // Special cases first:
    //
    if(p === 0) {
    ...
    // we have to solve:
    //
    const f = GammaInvA.gamma_inva_t(z, (p < q) ? p : q, (p < q) ? false : true);
    //
    ...
    //
    const r = Toms748.bracket_and_solve_root(f, guess, factor, false, tol, max_iter);
  ...
  public static gamma_p_inva(x: number, p: number): number {
  ...
  public static gamma_q_inva(x: number, q: number): number {
    // TODO: This border handling doesn't seem to make sense
    if (q === 1) {
      throw new Error(`Overflow error`);
    }
