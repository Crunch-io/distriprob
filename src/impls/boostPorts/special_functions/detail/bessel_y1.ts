...
export class BesselY1 {

  public static bessel_y1(x: number): number {
    const P1: number[] = [
    ...
    const x1  =  2.1971413260310170351e+00;
    const x2  =  5.4296810407941351328e+00;
    const x11 =  5.620e+02;
    const x12 =  1.8288260310170351490e-03;
    const x21 =  1.3900e+03;
    const x22 = -6.4592058648672279948e-06;

    let value, factor, r, rc, rs;

    if (x <= 0) {
      throw new Error(`Domain error: Got x = ${x}, but x must be > 0, complex${""
      } result not supported.`);
    }

    if (x <= 4) {                       // x in (0, 4]
      const y = x * x;
      const z = 2 * Math.log(x/x1) * BesselJ1.bessel_j1(x) / Math.PI;
      r = Rational.evaluate_rational(P1, Q1, y);
      factor = (x + x1) * ((x - x11/256) - x12) / x;
      value = z + factor * r;
    } else if (x <= 8) {                  // x in (4, 8]
      const y = x * x;
      const z = 2 * Math.log(x/x2) * BesselJ1.bessel_j1(x) / Math.PI;
      ...
      factor = 1 / (Math.sqrt(x) * Constants.SQRTPI());
      //
