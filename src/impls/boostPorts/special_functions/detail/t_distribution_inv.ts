...
const IBetaInverse = require("./ibeta_inverse").IBetaInverse;


export class InverseStudentsT {

  private static inverse_students_t_hill(ndf: number, u: number): number {
    let a, b, c, d, q, x, y;
  ...
  private static inverse_students_t_tail_series(df: number, v: number): number {
    // Tail series expansion, see section 6 of Shaw's paper.
    // w is calculated using Eq 60:
    let w = Gamma.tgamma_delta_ratio(df / 2, 0.5) * Math.sqrt(df * Math.PI) * v;
    ...
    //
    const rn = Math.sqrt(df);
    const div = Math.pow(rn * w, 1 / df);
    const power = div * div;
    let result = Rational.evaluate_polynomial(d, power);
    result *= rn;
    result /= div;
    return -result;
  }

  private static inverse_students_t_body_series(df: number, u: number): number {
    //
    // Body series for small N:
    //
    // Start with Eq 56 of Shaw:
    //
    let v = Gamma.tgamma_delta_ratio(df / 2, 0.5) * Math.sqrt(df * Math.PI)*(u-0.5);
    //
  ...
  private static inverse_students_t(df: number, u: number, v: number):
  {result: number, pexact: boolean} {
    //
    ...
    if((Math.floor(df) === df) && (df === 1 || df === 2 || df === 4 || df === 6)) {
      ...
      // cases first:
      //
      let tolerance = Math.pow(2, (2 * 53) / 3);
      ...
      switch(Math.trunc(df)) {
        ...
        case 4:
          ...
          //
          const alpha = 4 * u * v;
          const root_alpha = Math.sqrt(alpha);
          const r = 4 * Math.cos(Math.acos(root_alpha) / 3) / root_alpha;
          const x = Math.sqrt(r - 4);
          result = u - 0.5 < 0 ? -x : x;
          pexact = true;
          break;
        ...
        case 6:
          ...
          // supplement:
          //
          const a = 4 * (u - u * u);//1 - 4 * (u - 0.5f) * (u - 0.5f);
          const b = Math.cbrt(a);
      ...
      if(df > 0x10000000) {
        ...
        //
        const crossover = 0.2742 - df * 0.0242143;
        if(u > crossover) {
        ...
        //
        const crossover = Math.pow(2, Math.round(df / -0.654));
        if(u > crossover) {
  ...
  public static find_ibeta_inv_from_t_dist(a: number, p: number, q: number):
  {result: number, py: number} {
    const u = p / 2;
    const v = 1 - u;
    const df = a * 2;
    const t = InverseStudentsT.inverse_students_t(df, u, v).result;
    const py = t * t / (df + t * t);
    return {result: df / (df + t * t), py: py};
  }

  private static fast_students_t_quantile_mlp_false(df: number, p: number): number {
    //
    // Need to use inverse incomplete beta to get
    // required precision so not so fast:
    //
    let probability = (p > 0.5) ? 1 - p : p;
    let t, x;
    let y = 0;
    let iBetaInverseEval = IBetaInverse.ibeta_inv(
      df / 2,
      0.5,
      2 * probability,
    );
  ...
  public static fast_students_t_quantile(df: number, p: number): number {
    let invert: boolean = false;
    ...
    //
    let estimate = InverseStudentsT.inverse_students_t(df, p, 1-p);
    let exact: boolean = estimate.pexact;
    let t = estimate.result;
    if ((t === 0) || exact) {
    ...
    let f1 = resultObj.pderivative;
    let f0 = resultObj.result;

    // Get cdf from incomplete beta result:
    let p0 = f0 / 2  - p;
    // Get pdf from derivative:
    let p1 = f1 * Math.sqrt(y * xb * xb * xb / df);
    //
