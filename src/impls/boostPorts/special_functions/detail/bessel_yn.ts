...
const BesselY0 = require("./bessel_y0").BesselY0;
const BesselY1 = require("./bessel_y1").BesselY1;


export class BesselYN {

  public static bessel_yn(n: number, x: number): number {
    let value, factor, current, prev;
    ...
    if (x <= 0) {
      throw new Error(`Domain error: Got x = ${x}, but x must be > 0, complex result${""
      } not supported.`);
    }
    ...
    if(x < Precision.epsilon()) {
      let scale = 1;
      let resultObj = BesselJYSeries.bessel_yn_small_z(n, x, scale);
