...
const Bernoulli = require("./bernoulli").Bernoulli;
const Zeta = require("./zeta").Zeta;
const CosPI = require("./cos_pi").CosPI;
const Rational = require("../tools/rational").Rational;
const Sign = require("./sign").Sign;
const Digamma = require("./digamma").Digamma;


export class Polygamma {

  /**
   * for large values of x such as for x > 400
   */
  private static polygamma_atinfinityplus(n: number, x: number): number {
  ...
  private static polygamma_attransitionplus(n: number, x: number): number {
    // See: http://functions.wolfram.com/GammaBetaErf/PolyGamma2/16/01/01/0017/

    // Use N = (0.4 * digits) + (4 * n) for target value for x:
    const d4d  =  6;    // 0.4F * policies::digits_base10<T, Policy>();
    const N = d4d + (4 * n);
    const m    = n;
    const iter = N - Math.trunc(x);
    const max_iter = 50000;

    if(iter > max_iter) {
      throw new Error(`Evaluation error: Exceeded maximum series evaluations${""
      } evaluating at n = ${n} and x = ${x}`);
    }

    const minus_m_minus_one = -m - 1;

    let z = x;
    let sum0 = 0;
    let z_plus_k_pow_minus_m_minus_one = 0;
    ...
    if(Math.log(z + iter) * minus_m_minus_one > -Precision.log_max_value()) {
      ...
      for(let k = 1; k <= iter; k++) {
        let log_term = Math.log(z) * minus_m_minus_one + Gamma.lgamma(n + 1).result;
  ...
  private static polygamma_nearzero(n: number, x: number): number {
    //
    ...
    while(true) {
      // Get the k'th term:
      let term = factorial_part * Zeta.zeta(k + n + 1);
      sum += term;
  ...
  private static poly_cot_pi(n: number, x: number, xc: number): number {
    ...
    const LOGS_CUTOFF = 171;
    const MAX_N = 5000;



    const useLogs: boolean = n > LOGS_CUTOFF;
    const s = Math.abs(x) < Math.abs(xc) ? SinPI.sin_pi(x) : SinPI.sin_pi(xc);
    const c = CosPI.cos_pi(x);
    const P: number[] = Polygamma.coefs(n, useLogs);

    if (!useLogs) {
      const result = Math.pow(Math.PI, n) * (n % 2 === 0 ? c : 1) *
    ...
    //
    if(n > MAX_N) {
      throw new Error(`Evaluation error: The value of n is so large that we're${""
        } unable to compute the result in reasonable time, best guess is ${0}`);
    }

    let sum = Rational.evaluate_even_polynomial(P, c);
    ...
    //
    let power_terms = n * Constants.LNPI();
  ...
  public static coefs(n: number, scaled: boolean): number[] {
    const table: number[][] = [[-1]];
    const index = n - 1;

    if(index >= table.length) {
      for(let i = table.length - 1; i < index; ++i) {
        let offset = i & 1; // 1 if the first cos power is 0, otherwise 0.
        let sin_order = i + 2;  // order of the sin term
        let max_cos_order = sin_order - 1;  // largest order of the polynomial of cos
                                            // terms
        let max_columns = Math.trunc((max_cos_order - offset) / 2);  // How many entries
                                                                     // there are in the
                                                                     // current row.
        let next_offset = offset ? 0 : 1;
        // How many entries there will be in the next row
        let next_max_columns = Math.trunc((max_cos_order + 1 - next_offset) / 2);

        // add array of next_max_columns+1 0's to table
        table.push(Array.apply(null, Array(next_max_columns+1)).map(
          Number.prototype.valueOf,
          0
        ));

        for(let column = 0; column <= max_columns; ++column) {
          let cos_order = 2 * column + offset;  // order of the cosine term in entry
                                                // "column"
  ...
  private static polygamma_imp(n: number, x: number): number {
    if(n < 0) {
    ...
    if(x < 0) {
      ...
      const z = 1 - x;
      const result = Polygamma.polygamma_imp(n, z) +
    ...
    //
    const small_x_limit = Math.min(5 / n, 0.25);
    if(x < small_x_limit) {
      ...
      let result = (n & 1 ? 1 : -1) * UncheckedFactorial.unchecked_factorial(n) *
        Zeta.zeta(n + 1);
  ...
  public static polygamma(n: number, x: number): number {
    //
