...
var Erf = require("./boostPorts/special_functions/erf").Erf;
var ErfInv = require("./boostPorts/special_functions/detail/erf_inv").ErfInv;
var check = require("./errorHandling").check;
var NormalDist = /** @class */ (function () {
    function NormalDist() {
    }
    NormalDist.checkParameters = function (functionName, mu, sigma, x, lowerTail, p, n, seed) {
        var params = {
            mu: NaN,
            sigma: NaN,
            lowerTail: undefined
    ...
    NormalDist.pdf = function (x, mu, sigma) {
        var params = NormalDist.checkParameters("pdf", mu, sigma, x);
        if (x === Number.POSITIVE_INFINITY || x === Number.NEGATIVE_INFINITY) {
            return 0; // pdf + and - infinity is zero.
        }
        var result;
        var exponent = x - params.mu;
        exponent *= -exponent;
    ...
    NormalDist.cdf = function (x, mu, sigma, lowerTail) {
        var params = NormalDist.checkParameters("cdf", mu, sigma, x, lowerTail);
        ...
        var diff = (x - params.mu) / (params.sigma * Constants.SQRT2());
        if (params.lowerTail) {
    ...
    NormalDist.quantile = function (p, mu, sigma, lowerTail) {
        var params = NormalDist.checkParameters("quantile", mu, sigma, undefined, lowerTail, p);
        if (p === 0) {
            if (params.lowerTail) {
            ...
            var result = ErfInv.erfc_inv(2 * p);
            if (params.lowerTail) {
                result = -result;
            }
    ...
    NormalDist.random = function (n, mu, sigma, seed) {
        var params = NormalDist.checkParameters("random", mu, sigma, undefined, undefined, undefined, n, seed);
        return Random.numbers(n, NormalDist.quantile, [params.mu, params.sigma, true], seed);
    };
    NormalDist.randomIterator = function (n, mu, sigma, seed) {
        if (typeof n === "undefined" || n === null) {
            n = Number.POSITIVE_INFINITY;
        }
        var params = NormalDist.checkParameters("random", mu, sigma, undefined, undefined, undefined, n, seed);
        return Random.numberIterator(n, NormalDist.quantile, [params.mu, params.sigma, true], seed);
    };
    return NormalDist;
}());
