 ...
 *
 */
var HypergeometricPDF = require("./hypergeometric_pdf").HypergeometricPDF;
var Gamma = require("../../special_functions/gamma").Gamma;
var Precision = require("../../tools/precision").Precision;
var HypergeometricQuantile = /** @class */ (function () {
    function HypergeometricQuantile() {
    }
    HypergeometricQuantile.round_x_from_p = function (x, p, cum, fudge_factor, lbound, ubound, round) {
        if (round === "up") {
    ...
    HypergeometricQuantile.round_x_from_q = function (x, q, cum, fudge_factor, lbound, ubound, round) {
        if (round === "up") {
    ...
    HypergeometricQuantile.imp = function (p, q, r, n, N, round, cache) {
        var result;
        var fudge_factor = 1 + Precision.epsilon() * ((N <= cache.primes.maxLimit) ? 50 : 2 * N);
        var base = Math.max(0, Math.trunc(n + r) - Math.trunc(N));
        var lim = Math.min(r, n);
        if (p <= 0.5) {
            var x = base;
            ...
            if (diff === 0) {
                ...
                var log_pdf = Gamma.lgamma(n + 1).result
            ...
            return HypergeometricQuantile.round_x_from_p(x, p, result, fudge_factor, base, lim, round);
        }
        else {
            var x = lim;
            result = 0;
            var diff = HypergeometricPDF.imp(x, r, n, N, cache);
            if (diff === 0) {
                // We want to skip through x values as fast as we can until we start getting non-zero values,
                // otherwise we're just making lots of expensive PDF calls:
                x--;
            ...
            return HypergeometricQuantile.round_x_from_q(x, q, result, fudge_factor, base, lim, round);
        }
    };
    return HypergeometricQuantile;
}());
exports.HypergeometricQuantile = HypergeometricQuantile;
