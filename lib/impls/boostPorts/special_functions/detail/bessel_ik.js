...
var BesselIK = /** @class */ (function () {
    function BesselIK() {
    }
    BesselIK.cyl_bessel_i_small_z = function (v, z) {
        var k, mult, term, result;
    ...
    BesselIK.bessel_i_small_z_series = function (v, x) {
        var prefix;
        ...
        var s = BesselIK.cyl_bessel_i_small_z(v, x);
        var max_iter = 500;
     ...
     */
    BesselIK.temme_ik = function (v, x) {
        var f, h, p, q, coef, sum, sum1, tolerance;
        var a, b, c, d, sigma, gamma1, gamma2;
        var k;
     ...
     */
    BesselIK.CF2_ik = function (v, x) {
        var S, C, Q, D, f, a, b, q, delta, tolerance, current, prev;
        var k;
     ...
     */
    BesselIK.bessel_ik = function (v, x, kind) {
        // Kv1 = K_(v+1), fv = I_(v+1) / I_v
        // Ku1 = K_(u+1), fu = I_(u+1) / I_u
        var need_i = 1;
        var need_k = 2;
        var _kind = kind === "need_i" ? need_i : need_k;
        var u, Iv, Kv, Kv1, Ku, Ku1, fv;
        var W, current, prev, next;
        var reflect = false;
        var n, k;
        var org_kind = _kind;
        var I, K;
        ...
        u = v - n; // -1/2 <= u < 1/2
        if (x < 0) {
            throw new Error("Domain error: Got x = " + x + " but real argument x must be" + "" + " non-negative, complex number result not supported.");
        }
        if (x === 0) {
            Iv = (v === 0) ? 1 : 0;
            if (_kind & need_k) {
        ...
        W = 1 / x; // Wronskian
        if (x <= 2) {
            var resultObj = BesselIK.temme_ik(u, x); // Temme series
            Ku = resultObj.K;
            Ku1 = resultObj.K1;
        }
        else {
            var resultObj = BesselIK.CF2_ik(u, x); // continued fraction CF2_ik
            Ku = resultObj.Kv;
            Ku1 = resultObj.Kv1;
        }
        prev = Ku;
        ...
        if (reflect) {
            var z = (u + n % 2);
            var fact = (2 / Math.PI) * (SinPI.sin_pi(z) * Kv);
            if (fact === 0) {
                I = Iv;
            }
