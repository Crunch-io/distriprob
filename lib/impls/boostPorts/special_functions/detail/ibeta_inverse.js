...
var IGammaInverse = require("./igamma_inverse").IGammaInverse;
var Beta = require("../beta").Beta;
var InverseStudentsT = require("./t_distribution_inv").InverseStudentsT;
var Expm1 = require("../expm1").Expm1;
var Log1p = require("../log1p").Log1p;
var Precision = require("../../tools/precision").Precision;
var IBetaInverse = /** @class */ (function () {
    function IBetaInverse() {
    }
    IBetaInverse.temme_root_finder = function (t, a) {
        return function (x) {
            var y = 1 - x;
            if (y === 0) {
                var big = Number.MAX_VALUE / 4;
            ...
            var f0 = Math.log(x) + a * Math.log(y) + t;
            var f1 = (1 / x) - (a / (y));
            return { f0: f0, f1: f1 };
        };
    };
     ...
     * Section 2.
     */
    IBetaInverse.temme_method_1_ibeta_inverse = function (a, b, z) {
        var r2 = Constants.SQRT2();
        //
        // get the first approximation for eta from the inverse
        // error function (Eq: 2.9 and 2.10).
        //
        var eta0 = ErfInv.erfc_inv(2 * z);
        eta0 /= -Math.sqrt(a / 2);
        var terms = [eta0];
        var workspace1 = [];
        var workspace2 = [];
        ...
        //
        var eta = Rational.evaluate_polynomial(terms, 1 / a);
        //
        // now we need to convert eta to x, by solving the appropriate
        // quadratic equation:
        //
        var eta_2 = eta * eta;
        var c = -Math.exp(-eta_2 / 2);
        var x;
        if (eta_2 === 0) {
            x = 0.5;
        }
     ...
     * Section 3.
     */
    IBetaInverse.temme_method_2_ibeta_inverse = function (a, b, z, r, theta) {
        //
        ...
        //
        var eta = Rational.evaluate_polynomial(terms, 1 / r);
        //
        ...
        var alpha = c / s;
        alpha *= alpha;
        var lu = (-(eta * eta) / (2 * s_2) + Math.log(s_2) + c_2 * Math.log(c_2) / s_2);
        //
        ...
        // And iterate:
        //
        x = Roots.newton_raphson_iterate(IBetaInverse.temme_root_finder(-lu, alpha), x, lower, upper, 53 / 2, // <== Todo: make sure this is OK
        500);
        return x.result;
    };
    /**
     ...
     * Section 4.
     */
    IBetaInverse.temme_method_3_ibeta_inverse = function (a, b, p, q) {
        //
        ...
        e1 -= (75 * w_6 + 202 * w_5 + 188 * w_4 - 888 * w_3 - 1345 * w_2 + 118 * w + 138) *
            d_4 / (272160 * w1_4 * w_5);
        var e2 = (28 * w_4 + 131 * w_3 + 402 * w_2 + 581 * w + 208) * (w - 1) /
            (1620 * w1 * w_3);
        ...
        var e3 = -((3592 * w_7 + 8375 * w_6 - 1323 * w_5 - 29198 * w_4 - 89578 * w_3 -
        ...
        //
        var eta = eta0 + e1 / a + e2 / (a * a) + e3 / (a * a * a);
        //
        ...
        var u = eta - mu * Math.log(eta) + (1 + mu) * Math.log(1 + mu) - mu;
        var cross = 1 / (1 + mu);
        var lower = eta < mu ? cross : 0;
        var upper = eta < mu ? 1 : cross;
        var x = (lower + upper) / 2;
        var nrResult = Roots.newton_raphson_iterate(IBetaInverse.temme_root_finder(u, mu), x, lower, upper, 53 / 2, // <== todo: make sure this is ok
        500);
        x = nrResult.result;
        return x;
    };
    IBetaInverse.ibeta_roots = function (a, b, t, inv) {
        if (inv === void 0) { inv = false; }
        var target = t;
        var invert = inv;
        return function (x) {
            var y = 1 - x;
            var iBetaEval = Beta.ibeta_imp(a, b, x, invert, true);
            var f1 = iBetaEval.pderivative;
            var f = iBetaEval.result - target;
            if (invert) {
                f1 = -f1;
            }
            if (y === 0) {
    ...
    IBetaInverse.ibeta_inv_imp = function (a, b, p, q) {
        //
        ...
        //
        var x = 0; // Set to a safe zero to avoid a
        ...
        if ((b === 0.5) && (a >= 0.5) && (p !== 1)) {
            ...
            var resultObj = InverseStudentsT.find_ibeta_inv_from_t_dist(a, p, q);
            x = resultObj.result;
            y = resultObj.py;
        }
        else if (b === 1) {
            ...
            if ((Math.sqrt(minv) > (maxv - minv)) && (minv > 5)) {
                ...
                var r = a + b;
                var theta = Math.asin(Math.sqrt(a / r));
                var lambda = minv / r;
            ...
            var xg = Math.pow(a * p * Beta.beta(a, b), 1 / a);
            x = xg / (1 + xg);
            y = 1 / (1 + xg);
            //
            ...
            //
            var lx = Math.log(p * a * Beta.beta(a, b)) / a;
            x = Math.exp(lx);
        ...
        //
        var max_iter = 500;
        var halleyResult = Roots.halley_iterate(IBetaInverse.ibeta_roots(a, b, (p < q ? p : q), (p < q ? false : true)), x, lower, upper, digits, max_iter);
    ...
    IBetaInverse.ibeta_inv = function (a, b, p) {
        if (a <= 0) {
            throw new Error("Domain error: The argument a to the incomplete beta function" + "" + " inverse must be greater than zero (got a=" + a + ").");
        }
        if (b <= 0) {
            throw new Error("Domain error: The argument b to the incomplete beta function" + "" + " inverse must be greater than zero (got b=" + b + ").");
        }
        if ((p < 0) || (p > 1)) {
            throw new Error("Domain error: Argument p outside the range [0,1] in the" + "" + " incomplete beta function inverse (got p=" + p + ").");
        }
        var resultObj = IBetaInverse.ibeta_inv_imp(a, b, p, 1 - p);
        var rx = resultObj.result;
        var ry = resultObj.py;
        return { result: rx, py: ry };
    };
    IBetaInverse.ibetac_inv = function (a, b, q) {
        if (a <= 0) {
            throw new Error("Domain error: The argument a to the incomplete beta function" + "" + " inverse must be greater than zero (got a=" + a + ").");
        }
        if (b <= 0) {
        ...
        if ((q < 0) || (q > 1)) {
            throw new Error("Domain error: Argument q outside the range [0,1] in the" + "" + " incomplete beta function inverse (got q=" + q + ").");
        }
        var resultObj = IBetaInverse.ibeta_inv_imp(a, b, 1 - q, q);
        var rx = resultObj.result;
        var ry = resultObj.py;
        return { result: rx, py: ry };
    };
...
exports.IBetaInverse = IBetaInverse;
