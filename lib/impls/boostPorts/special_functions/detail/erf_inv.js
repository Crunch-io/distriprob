...
var ErfInv = /** @class */ (function () {
     ...
     */
    ErfInv.erf_inv_imp = function (p, q) {
        var result = 0;
        if (p <= 0.5) {
            //
            ...
            var g = p * (p + 10);
            var r = Rational.evaluate_polynomial(P, p) /
                Rational.evaluate_polynomial(Q, p);
            result = g * Y + g * r;
        }
        else if (q >= 0.25) {
            //
            ...
            var xs = q - 0.25;
            var r = Rational.evaluate_polynomial(P, xs) /
                Rational.evaluate_polynomial(Q, xs);
            result = g / (Y + r);
        }
        else {
            //
            ...
            if (x < 3) {
                ...
                var xs = x - 6;
                var R = Rational.evaluate_polynomial(P, xs) / Rational.evaluate_polynomial(Q, xs);
                result = Y * x + R * x;
            }
            else if (x < 44) {
    ...
    ErfInv.erfc_inv = function (z) {
        //
        // Begin by testing for domain errors, and other special cases:
        //
        if ((z < 0) || (z > 2)) {
            throw new Error("Domain error: Argument outside range [0,2] in inverse erfc" + "" + " function (got p=" + z + ").");
        }
        if (z === 0 || z === 2) {
        ...
        //
        return s * ErfInv.erf_inv_imp(p, q);
    };
    ErfInv.erf_inv = function (z) {
        //
        // Begin by testing for domain errors, and other special cases:
        //
        if ((z < -1) || (z > 1)) {
            throw new Error("Domain error: Argument outside range [-1, 1] in inverse erf" + "" + " function (got p=" + z + ").");
        }
        if (z === 1 || z === -1) {
            throw new Error("Overflow error");
        }
