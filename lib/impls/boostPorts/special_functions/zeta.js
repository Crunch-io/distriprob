...
var Zeta = /** @class */ (function () {
    function Zeta() {
    }
    Zeta.zeta_polynomial_series = function (s, sc) {
        //
        ...
        //
        var n = Math.trunc(Math.log(Precision.epsilon()) / -2);
        var sum = 0;
        var two_n = Math.pow(2, n);
        var ej_sign = 1;
        for (var j = 0; j < n; j++) {
     ...
     */
    Zeta.zeta_imp_prec = function (s, sc) {
        var result;
        if (s < 1) {
    ...
    Zeta.zeta_imp_odd_integer = function (s) {
        var results = [
            1.2020569031595942,
            1.03692775514337,
            1.008349277381923,
        ...
        if (Math.abs(s) < Precision.root_epsilon()) {
            ...
            if (Math.floor(sc / 2) === sc / 2) {
                ...
                if (s > UncheckedFactorial.max_factorial()) {
                    var mult = SinPI.sin_pi(0.5 * sc) * 2 * Zeta.zeta_imp(s, sc);
