...
var OwensT = /** @class */ (function () {
     ...
     */
    OwensT.owens_t_compute_code = function (h, a) {
        var hrange = [0.02, 0.06, 0.09, 0.125, 0.26, 0.4, 0.6, 1.6, 1.7, 2.33, 2.4, 3.36, 3.4, 4.8];
        var arange = [0.025, 0.09, 0.15, 0.36, 0.5, 0.9, 0.99999];
        /*
    ...
    OwensT.owens_t_get_order = function (icode) {
        // 18 entries, 53 bit mantissa implementation
        var ord = [2, 3, 4, 5, 7, 10, 12, 18, 10, 20, 30, 0, 4, 7, 8, 20, 0, 0];
        return ord[icode];
    };
    /**
     * compute the value of Owen's T function with method T1 from the reference paper
     */
    OwensT.owens_t_T1 = function (h, a, m) {
        var hs = -h * h * 0.5;
        var dhs = Math.exp(hs);
        var as = a * a;
        var j = 1;
        var jj = 1;
        var aj = a * Constants.ONEDIVTWOPI();
        var dj = Expm1.expm1(hs);
        var gj = hs * dhs;
        var val = Math.atan(a) * Constants.ONEDIVTWOPI();
        while (true) {
            val += dj * aj / jj;
            if (m <= j) {
                break;
     ...
     */
    OwensT.owens_t_T2 = function (h, a, m, ah) {
        var maxii = m + m + 1;
        var hs = h * h;
        var as = -a * a;
        var y = 1 / hs;
        var ii = 1;
        var val = 0;
        var vi = a * Math.exp(-ah * ah * 0.5) * Constants.ONEDIVROOTTWOPI();
        var z = OwensT.owens_t_znorm1(ah) / h;
        while (true) {
            val += z;
            if (maxii <= ii) {
     ...
     */
    OwensT.owens_t_T3 = function (h, a, ah) {
        var m = 20;
        var c2 = [
        ...
        var i = 0;
        var vi = a * Math.exp(-ah * ah * 0.5) * Constants.ONEDIVROOTTWOPI();
        var zi = OwensT.owens_t_znorm1(ah) / h;
        var val = 0;
        while (true) {
            val += zi * c2[i];
            if (m <= i) {
     ...
     */
    OwensT.owens_t_T4 = function (h, a, m) {
        var maxii = m + m + 1;
        var hs = h * h;
        var as = -a * a;
        var ii = 1;
        var ai = a * Math.exp(-hs * (1 - as) * 0.5) * Constants.ONEDIVTWOPI();
        var yi = 1;
        var val = 0;
        while (true) {
            val += ai * yi;
     ...
     * 53 bit mantissa implementation
     */
    OwensT.owens_t_T5 = function (h, a) {
        /*
         NOTICE:
     ...
     */
    OwensT.owens_t_T6 = function (h, a) {
        var normh = OwensT.owens_t_znorm2(h);
        var y = 1 - a;
        var r = Math.atan2(y, 1 + a); // Todo: make sure atan2 is ok
        var val = normh * (1 - normh) * 0.5;
        if (r != 0) {
            val -= r * Math.exp(-y * h * h * 0.5 / r) * Constants.ONEDIVTWOPI();
        }
        return val;
    };
    OwensT.owens_t_T1_accelerated = function (h, a) {
        //
        ...
        var a_pow = a;
        var aa = a * a;
        var exp_term = Math.exp(-h * h / 2);
        var one_minus_dj_sum = exp_term;
        var sum = a_pow * exp_term;
        var dj_pow = exp_term;
        var term = sum;
        var abs_err;
        var j = 1;
        ...
        //
        var n = Math.trunc(Precision.log_max_value() / 6);
        var d = Math.pow(3 + Math.sqrt(8), n);
        d = (d + 1 / d) / 2;
        var b = -1;
        var c = -d;
        c = b - c;
    ...
    OwensT.owens_t_T2_mlp_true = function (h, a, m, ah) {
        var maxii = m + m + 1;
        var hs = h * h;
        var as = -a * a;
        var y = 1 / hs;
        var ii = 1;
        var val = 0;
        var vi = a * Math.exp(-ah * ah * 0.5) / Constants.SQRTTWOPI();
        var z = OwensT.owens_t_znorm1(ah) / h;
        var last_z = Math.abs(z);
        var lim = Precision.epsilon();
        while (true) {
            val += z;
            //
    ...
    OwensT.owens_t_T2_accelerated = function (h, a, ah) {
        //
    ...
    OwensT.T4_mp = function (h, a) {
        var hs = h * h;
        var as = -a * a;
        var ii = 1;
        var ai = Constants.ONEDIVTWOPI() * a * Math.exp(-0.5 * hs * (1.0 - as));
        var yi = 1.0;
        var val = 0.0;
        var lim = Precision.epsilon();
        var max_iter = 1500; // set explicitly
        while (true) {
            var term = ai * yi;
            val += term;
     ...
     */
    OwensT.owens_t_dispatch = function (h, a, ah) {
        //
        ...
        var val;
        var icode = OwensT.owens_t_compute_code(h, a);
        var m = OwensT.owens_t_get_order(icode);
        var meth = [1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 3, 4, 4, 4, 4, 5, 6]; // 18 entries
    ...
    OwensT.owens_t = function (h, a) {
        ...
        // computation routine.
        var fabs_a = Math.abs(a);
        var fabs_ah = fabs_a * h;
        var val;
        if (fabs_a <= 1) {
            ...
            if (h <= 0.67) {
                var normh = OwensT.owens_t_znorm1(h);
                var normah = OwensT.owens_t_znorm1(fabs_ah);
                val = 0.25 - normh * normah - OwensT.owens_t_dispatch(fabs_ah, 1 / fabs_a, h);
            }
            else {
                var normh = OwensT.owens_t_znorm2(h);
                var normah = OwensT.owens_t_znorm2(fabs_ah);
